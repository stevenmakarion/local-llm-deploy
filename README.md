...
## Measure it yourself: `numa_bench.py`

Do not take our numbers. Take the method. [`numa_bench.py`](numa_bench.py) runs a
STREAM-style multi-threaded copy pinned to each node in turn, then reports what your machine
actually does and what follows from it.

![](images/numa-bench.png)

```
MEMORY BANDWIDTH (GB/s, read+write, best of 3)
...
Three findings came out of that, and two of them surprised us.

### 1. Local is 2.6x faster than cross-node

38.4 against 14.6 GB/s. This is the one everybody knows about, and it is why the model, its
threads and its GPU all belong on one node.

### 2. `--interleave=all` is 23% SLOWER than pinning

This is the one worth the whole benchmark. The standard NUMA advice is to interleave, and it
is good advice for a workload spread evenly across sockets. **It is the wrong advice here.**
Interleaving averages the fast bank and the slow one and hands you the mean (29.7) when you
could have had the maximum (38.4).

If you followed the common guidance without measuring, you would leave 23% on the floor and
never know, because nothing would look broken.

### 3. The two nodes differ by 48%

38.4 against 26.0 GB/s, same machine, same DIMM type. The banks are populated
asymmetrically: 94 GiB on node 0, 63 GiB on node 1. **Fewer populated channels, less
bandwidth.**

Which means the choice of node is not arbitrary and it is not about capacity. Put the model
on the node with the most channels populated, then keep everything it touches there.

**Single-threaded measurement will lie to you about all of this.** A single thread on this
box reports 19.7 GB/s, because it is measuring one core's load/store ceiling rather than the
memory system. Benchmark with enough threads to saturate, or you will tune against the wrong
number.
...
**What that is worth, measured twice, minutes apart on the live server:**

| | prompt | generated | wall clock | tok/s |
|---|---|---|---|---|
| cache cold | 26 tok | 66 tok | **30.5s** | 2.16 |
| cache warm | 26 tok | 75 tok | **4.6s** | **16.27** |

Same model, same machine, same question. **7.5x the throughput and 85% off the wall clock**,
purely from whether the prefix was resident.

Note what that means for a *user-facing* product: the difference between a reply that reads
as instant and one that reads as broken is not the model you chose. It is whether something
touched the cache in the last half hour.
...
Set it per-service with `CUDA_VISIBLE_DEVICES` in the unit file, not in a shell profile, services do not read your `.bashrc`. Before the lanes were explicit, a heavy image job would
